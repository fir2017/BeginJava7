...
public class GatheringWrite { 
  public static void main(String[] args) {
    ...
    try(GatheringByteChannel fw = (FileChannel)(Files.newByteChannel(fp, WRITE, CREATE))){
      ByteBuffer[] buf = { ByteBuffer.allocate(8),null,ByteBuffer.allocate(8)};
      String pstr = null;
      for(Long p : primers) {
        pstr = "Primer = " + p; 
        buf[0].putDouble((double)(pstr.length())).flip();
        buf[1] = ByteBuffer.allocate(pstr.length() * 2);
        buf[1].put(pstr.getBytes()).flip();
        buf[2].putLong(p).flip();
        fw.write(buf);
        buf[0].clear();
        buf[2].clear();
      }
